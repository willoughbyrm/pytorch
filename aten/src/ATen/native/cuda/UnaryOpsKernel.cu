...
void nan_to_num_kernel_cuda(
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND1(kHalf, iter.dtype(), "nan_to_num_cuda", [&]() {
    scalar_t nan_replacement = static_cast<scalar_t>(nan.value_or(0.));
    scalar_t pos_inf_replacement = pos_inf.has_value()
        ? static_cast<scalar_t>(pos_inf.value())
        : std::numeric_limits<scalar_t>::max();
    std::cout << "pos inf: " << pos_inf_replacement << std::endl;
    std::cout << "nan repl: " << nan_replacement << std::endl;
     ...
     scalar_t num = at::_isnan(a)
      ...
      : (a == std::numeric_limits<scalar_t>::infinity()
             ...
             : (a == -std::numeric_limits<scalar_t>::infinity()
                    ...
                    : a));
      printf("compute num\n");
     // std::cout << "computed num: " << num << std::endl;
      return (num);
    });
  });
}
